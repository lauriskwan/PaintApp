class DrawingEllipse extends PaintFunction {
  ...
  onMouseDown(coord, event) {
    ...
    this.contextReal.fillStyle = currentColor; //canvas-configuration.js
    // this.contextDraft.strokeStyle = canvasSettings.colorStroke; //canvas-configuration.js
    this.contextDraft.fillStyle = currentColor; //canvas-configuration.js
  ...
  onDragging(coord, event) {
    ...
    this.contextDraft.ellipse(
      ...
      0,
      2 * Math.PI
    ); // rotation hardcoded as zero - essentially drawing an arc from zero @ x-axis to 2pi;
  ...
  onMouseUp(coord) {
    ...
    this.contextReal.ellipse(
      ...
      0,
      2 * Math.PI
    ); // rotation hardcoded as zero - use a JS text promt for user feedback?;
